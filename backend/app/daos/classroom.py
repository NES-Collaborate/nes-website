...
class ClassroomDao(BaseDao):
    def get_by_id(self, classroomId: int) -> Classroom:
        _classroom = (
            self.session.query(Classroom).filter_by(id=classroomId).first()
        )
    ...
    def get_members_by_id(
        ...
        if role:
            _members = _members.filter(Enrollment.role == role)

        _members = (
            _members.limit(page_size).offset((page - 1) * page_size).all()
        )
    ...
    def create_enrollment(
        self, userId: int, classroomId: int, role: str
    ) -> Enrollment:
        _enrollment = Enrollment(
            userId=userId, classroomId=classroomId, role=role
        )
